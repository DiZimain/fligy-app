(() => {
  ...
  function stageCopy(stage = growthStage()) {
    ...
    const doneToday = completedCardsToday();
    if (stage === "egg") return { title: "Яйцо еще спит", text: "Сегодня нужно закрыть три карточки. Как только сделаешь это, дракон вылупится в этот же день.", banner: `Под скорлупой зреет ${current.name}`, goal: "Выполнить 3 карточки за день", hint: `Сегодня выполнено: ${doneToday} из 3`, progress: doneToday === 0 ? "○ ○ ○ ○" : "★ ○ ○ ○" };
